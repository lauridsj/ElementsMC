...
public class DeepWaterRenderer implements ISimpleBlockRenderingHandler
{

	public static int getDensityDir(BlockFluidBase fluid)
	{
		return fluid.getFluid().getDensity() > 0 ? -1 : 1;
	}
    ...
    static final float BLOCK_HEIGHT = 8f / 9f;
    ...
    public float getFluidHeightForRender(IBlockAccess world, int x, int y, int z, BlockFluidBase block)
    {
        ...
        {
            if (world.getBlock(x, y - getDensityDir(block), z).getMaterial().isLiquid())
            {
                return 1;
            }
        ...
        return !world.getBlock(x, y, z).getMaterial().isSolid() && world.getBlock(x, y - getDensityDir(block), z) == block ? 1 : block.getQuantaPercentage(world, x, y, z) * BLOCK_HEIGHT;
    }
    ...
    public boolean renderWorldBlock(IBlockAccess world, int x, int y, int z, Block block, int modelId, RenderBlocks renderer)
    {
        ...
        boolean renderTop = block.shouldSideBeRendered(world, x, y - getDensityDir(theFluid), z, 1) && world.getBlock(x, y - getDensityDir(theFluid), z) != theFluid;

        boolean renderBottom = block.shouldSideBeRendered(world, x, y + getDensityDir(theFluid), z, 0) && world.getBlock(x, y + getDensityDir(theFluid), z) != theFluid;
        ...
        else
        {
            ...
            boolean rises = getDensityDir(theFluid) == 1;
